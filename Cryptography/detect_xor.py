...
def detect_encrypted_text(enc_strings):
  candidates = []

  for string in tqdm(enc_strings):
    candidates.append(single_byte_attack(string))

  return sorted(candidates, key=lambda x: x["score"], reverse=True)[0]


ciphertexts = [bytes.fromhex(line.strip())
               for line in open("data/4.txt")]

most_likely_plaintext = detect_encrypted_text(ciphertexts)

print("\n".join(f"{k.title()}\t{v}" for k, v in most_likely_plaintext.items()))
